...
\begin{lstlisting}
> putStrLn $ pr $ ruleFantasy (\pq -> ruleSepR pq >>= \q -> ruleSepL pq >>= \p -> ruleJoin q p) (And (PropVar A) (PropVar B))
|- A /\ B -> B /\ A
\end{lstlisting}
...
|- A=SS0 /\ 0=0 -> A=SS0
> putStrLn $ pr $ pre >>= \pre -> eg >>= \eg -> post >>= \post -> hoareConsequence pre eg post
...
{B=0 /\ A=A} C := A; {B=0 /\ C=A}
> putStrLn $ pr $ c1 >>= \c1 -> c2 >>= \c2 -> hoareSequence c1 c2
...
\begin{lstlisting}
> let prf1 = ruleFantasy (\pq -> ruleSepR pq >>= \q -> axiom1 (Var A) >>= \ax1 -> ruleSpec ax1 (Var A) >>= \ax1A -> ruleJoin ax1A q) (And (PropVar (Eq (Var A) Z)) (PropVar $ Eq Z Z))
|- A=0 /\ 0=0 -> ~SA=0 /\ 0=0
...
\begin{lstlisting}
> let eg3 = prf1 >>= \prf1 -> eg2 >>= \eg2 -> prf2 >>= \prf2 -> hoareConsequence prf1 eg2 prf2
{A=0 /\ 0=0} A := SA; {~A=0 /\ 0=0}
> putStrLn $ pr $ eg1 >>= \eg1 -> eg3 >>= \eg3 -> hoareConditional eg3 eg1
...
|- 0=0 -> 0=0
> let egTriple = pre >>= \pre -> c >>= \c -> post >>= \post -> hoareConsequence pre c post
{0=0 /\ 0=0} ; {0=0}
\end{lstlisting}
