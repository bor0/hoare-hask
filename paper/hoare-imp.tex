...
Note how we accept a non-proven term within \texttt{ruleFantasy}, whereas in other rules we accept proven terms; the hypothesis needn't be necessarily true, it only states that "If this hypothesis were a theorem, that would be a theorem". Further, we use the \texttt{Either} monad to cascade success or halt on failure. We also rely on Haskell's machinery for functions (lambda calculus) to take care of doing the actual work - we get support for \textbf{Carry Over Rule} out of the box as well.

\begin{lstlisting}
> putStrLn $ pr $ ruleFantasy (\pq -> join $ ruleJoin <$> ruleSepR pq <*> ruleSepL pq) (And (PropVar A) (PropVar B))
|- A /\ B -> B /\ A
\end{lstlisting}
...
\begin{lstlisting}
> let eg = hoareAssignment A (Plus (Var B) (S Z)) (And (PropVar (Eq (Var A) (S (S Z)))) (PropVar (Eq Z Z)))
{B+S0=SS0 /\ 0=0} A := B+S0; {A=SS0 /\ 0=0}
> let pre = ruleFantasy Right $ And (PropVar (Eq (Plus (Var B) (S Z)) (S (S Z)))) (PropVar (Eq Z Z))
|- B+S0=SS0 /\ 0=0 -> B+S0=SS0 /\ 0=0
> let post = ruleFantasy ruleSepL $ And (PropVar (Eq (Var A) (S (S Z)))) (PropVar (Eq Z Z))
|- A=SS0 /\ 0=0 -> A=SS0
> putStrLn $ pr $ join $ hoareConsequence <$> pre <*> eg <*> post
...
\begin{lstlisting}
> let eg = hoareAssignment A (Plus (Var B) (S Z)) (PropVar (Eq (Var A) (S (S Z))))
...
> let post = ruleFantasy Right $ PropVar (Eq (Var A) (S (S Z)))
|- A=SS0 -> A=SS0
> putStrLn $ pr $ join $ hoareConsequence <$> pre <*> eg <*> post
{B+S0=SS0 /\ 0=0} A := B+S0; {A=SS0}
\end{lstlisting}
...
\begin{lstlisting}
> let eg1 = hoareSkip (And (Not $ PropVar (Eq (Var A) Z)) (PropVar $ Eq Z Z))
{~A=0 /\ 0=0} ; {~A=0 /\ 0=0}
> let eg2 = hoareAssignment A (S (Var A)) (And (Not $ PropVar (Eq (Var A) Z)) (PropVar $ Eq Z Z))
...
\begin{lstlisting}
> let prf1 = ruleFantasy (\pq -> join $ ruleJoin <$> (axiom1 (Var A) >>= \ax1 -> ruleSpec ax1 (Var A)) <*> ruleSepR pq) (And (PropVar (Eq (Var A) Z)) (PropVar $ Eq Z Z))
|- A=0 /\ 0=0 -> ~SA=0 /\ 0=0
> let prf2 = ruleFantasy Right $ (And (Not $ PropVar (Eq (Var A) Z)) (PropVar $ Eq Z Z))
|- ~A=0 /\ 0=0 -> ~A=0 /\ 0=0
\end{lstlisting}
...
\begin{lstlisting}
> let eg3 = join $ hoareConsequence <$> prf1 <*> eg2 <*> prf2
{A=0 /\ 0=0} A := SA; {~A=0 /\ 0=0}
> putStrLn $ pr $ join $ hoareConditional <$> eg3 <*> eg1
...
\begin{lstlisting}
> let c = hoareSkip (PropVar $ Eq Z Z) :: Either String (HoareTriple Integer)
{0=0} ; {0=0}
> let pre = ruleFantasy ruleSepR (And (PropVar $ Eq Z Z) (PropVar $ Eq Z Z)) :: Either String (Proof (PropCalc (FOL Integer)))
|- 0=0 /\ 0=0 -> 0=0
> let post = ruleFantasy Right (PropVar $ Eq Z Z) :: Either String (Proof (PropCalc (FOL Integer)))
|- 0=0 -> 0=0
> let egTriple = join $ hoareConsequence <$> pre <*> c <*> post
{0=0 /\ 0=0} ; {0=0}
\end{lstlisting}
...
\begin{lstlisting}
> putStrLn $ pr $ egTriple >>= hoareWhile
...
pre = ruleFantasy f premise where
  ...
  f premise = go >>= \go -> ruleExistence go C [] where
    -- |- ~~Exists C:(A+C=B)
    go = ruleSepR premise >>= \premiseR -> ruleDoubleTildeIntro premiseR >>= \step1 ->
         -- |- ~~A+SC=B
