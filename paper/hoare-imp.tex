...
We list the rules of Hoare logic, some of them outlined in the original paper\cite{b7}. However, we will only provide an implementation for some of them.
...
Rules should not be manually constructed using \texttt{HoareTriple}, rather through the functions that we provide next.

\subsubsection{H-Skip}

We provide the Haskell implementation of the Hoare skip rule.
...
We rely on the optimizing functions to further simplify calculations. Note that the validity of the precondition is not being checked (since that would require evaluating \texttt{Aexp}/\texttt{Bexp} with a context); the proof merely states that assuming some precondition, a command produces some postcondition.
...
In this case, the implementation will rewrite a variable with an arithmetical expression within a boolean expression. Given this definition, some proofs can be deduced, such as:
...
Given the precondition \texttt{TRUE} (\texttt{3 = 3} before optimization), the command \texttt{X := 3} implies the postcondition \texttt{X = 3}.
...
To be able to construct these proofs, we need to add another substitution rule:
...
This substitution rule states that whenever $x$ is zero, increasing it by a non-zero value will produce the postcondition that it is not zero. In addition, we need to expand \texttt{boptimize} with the following definition, so that it handles double negation properly:
...
Right {TRUE} (If (X == 0) Then (X := X + 1;) Else (;)); {! (X == 0) && 0 == 0}
\end{lstlisting}

\subsection{Example: Swap}

Even though our implementation and its substitution and optimization rules are simple enough, it can still prove some very useful facts. Consider the following proofs:
...
That is, we start with the precondition that \texttt{a == A \&\& b == B} and we reach \texttt{b == A \&\& a == B} using some set of commands - the function to swap the values between two variables.
